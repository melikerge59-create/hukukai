  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center h-16 px-4 border-b border-border-light dark:border-border-dark
        ...
        {!collapsed && (
          <span className="font-serif text-xl font-bold text-primary-600 dark:text-white">
            HukukAI
          </span>
        )}
      </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center rounded-xl text-sm font-medium transition-all duration-150
                  ...
                  ${active
                    ? 'sidebar-active text-primary-600 dark:text-primary-300'
                    : 'text-gray-500 dark:text-slate-400 hover:bg-primary-50 dark:hover:bg-white/5 hover:text-primary-600 dark:hover:text-white border-l-[3px] border-l-transparent'
                  }`}
              >
      ...
      <div className={`border-t border-border-light dark:border-border-dark p-3
        ...
        {collapsed ? (
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-DEFAULT to-primary-500
              flex items-center justify-center text-white text-sm font-bold shrink-0">
              {initials}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-gray-900 dark:text-white truncate">{displayName}</p>
              <p className="text-[10px] text-accent-500 font-medium">{planLabel} Plan</p>
            </div>
            <button
              onClick={() => signOut()}
              title="Çıkış Yap"
      ...
      <button
        ...
        className="absolute -right-3 top-20 w-6 h-6 rounded-full bg-white dark:bg-surface-dcard
          ...
          flex items-center justify-center text-gray-400 hover:text-primary-600
