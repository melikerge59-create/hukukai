...
module.exports = async function handler(req, res) {
  ...
  if (req.method !== 'POST') {
    return res.redirect(303, siteUrl);
  }

  const token  = req.body?.token;
  const status = req.body?.status;

  if (!token)            return res.redirect(303, siteUrl + '?payment=error');
  if (status === 'failure') return res.redirect(303, siteUrl + '?payment=failed');
  ...
  try {
    ...
    try { data = JSON.parse(text); } catch(e) {
      return res.redirect(303, siteUrl + '?payment=error');
    }

    if (data.paymentStatus !== 'SUCCESS' && data.status !== 'success') {
      return res.redirect(303, siteUrl + '?payment=failed');
    }

    // Plan bilgisini belirle
    const planId = data.itemTransactions?.[0]?.itemId || 'plus';
    ...
    if (userId) {
      await supabase
        ...
        .upsert({
          user_id: userId,
          plan_type: planId,
          daily_limit: planLimits[planId] || 50,
          is_active: true
        }, { onConflict: 'user_id' });
      console.log('Plan güncellendi:', userId, planId);
    }

    return res.redirect(303, siteUrl + '?payment=success&plan=' + planId);
